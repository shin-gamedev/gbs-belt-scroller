...
direction_e belt_dir = DIR_RIGHT; // added in part 3

UBYTE toggle_animation = 0; // added in part 4
UBYTE toggle_movement = 0; // added in part 4

void beltscroller_init(void) BANKED {
...
void beltscroller_update(void) BANKED {
    actor_t *hit_actor;
    ...
    if (INPUT_LEFT) {
        belt_dir = DIR_LEFT; // added in part 3
        player_moving = TRUE;
        if (INPUT_UP) {
            angle = ANGLE_315DEG;
        } else if (INPUT_DOWN) {
        ...
        belt_dir = DIR_RIGHT; // added in part 3
        player_moving = TRUE;
        if (INPUT_UP) {
            angle = ANGLE_45DEG;
        } else if (INPUT_DOWN) {
    ...
    if (player_moving) {
        ...
        // added in part 3 and 4
        if (toggle_movement == 0) {
            new_pos.y = (PLAYER.pos.y + new_pos.y) >> 1;
        }
...
// added in part 3 and updated in part 4
    if (new_dir != DIR_NONE) {
        if (toggle_animation == 0) {
            actor_set_dir(&PLAYER, belt_dir, player_moving);
        } else {
